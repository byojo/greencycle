...
Page({
  ...
  renderFromServer(order) {
    ...
    this.setData({
      ...
      amountText: order.status === 4 && order.finalAmount > 0 ? '回收金额 ' + formatMoney(order.finalAmount) : '',
      priceText: order.status === 4 && order.finalAmount > 0 ? '¥' + (Math.round((order.finalAmount || 0) / 100)).toLocaleString() : '',
